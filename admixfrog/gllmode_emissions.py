...
def update_Ftau_gllmode(F, tau, PG, P, IX):
    ...
    for s in range(n_states):
        ...
        def f(t):
            F, tau = t
            x = np.log(_p_gt_homo(s, P, F, exp(tau)) + 1e-10) * PG[:, s, :] * w
...
def update_geno_emissions(GT, P, IX, F, tau, n_states, est_inbreeding):
    ...
    if not est_inbreeding:
        ...
        for s in range(n_homo_states):
            a, b = P.alpha[IX.HAPSNP, s], P.beta[IX.HAPSNP, s] GT[IX.HAPSNP, s, 0] = b / (a + b)  # if a +b > 0 else 0 GT[IX.HAPSNP, s, 2] = a / (a + b)  # if a +b > 0 else 0 
    return GT
