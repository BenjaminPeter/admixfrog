...
def bins_from_bed(bed, data, bin_size):
    ...
    for i, chrom in enumerate(chroms):
        ...
        data_loc.append(np.vstack((np.zeros_like(dig_data)+i, dig_data+bin0, dig_data)).T)
...
def run_hmm(infile, bedfile, split_lib=True, 
    ...
    gamma_out = pd.DataFrame(gamma_out, columns = ["chrom_id", "bin_pos", "bin_id"] + gamma_names)
    gamma_out = pd.concat((gamma_out.reset_index(drop=True),
                           viterbi_df.reset_index(drop=True)), axis=1)
    data_out = pd.concat((data.reset_index(drop=True),
                   gamma_out.iloc[bin_data[:,1]].reset_index(drop=True)), axis=1)
