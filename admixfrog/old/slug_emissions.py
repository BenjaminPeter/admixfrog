...
def slug_bwd_p_o_given_gac_simple(bx):
    ...
    """
    res = np.empty((3, 2))
    res[0, 0] = bx[0,0] * 1.0 + bx[1,0] * 0.0      #Pr(O = 0 | G=0, C=0) = Pr(O=0 | A=0, C=1)
    res[0, 1] = bx[0,1] * 1.0 + bx[1,1] * 0.0      #Pr(O = 1 | G=0, C=0) = Pr(O=1 | A=0, C=1)

    res[1, 0] = bx[0,0] * 0.5 + bx[1,0] * 0.5      #Pr(O = 0 | G=1, C=0) 
    res[1, 1] = bx[0,1] * 0.5 + bx[1,1] * 0.5      #Pr(O = 1 | G=1, C=0) 

    res[2, 0] = bx[0,0] * 0.0 + bx[1,0] * 1.0      #Pr(O = 0 | G=2, C=0) = Pr(O=0 | A=1, C=1)
    res[2, 1] = bx[0,1] * 0.0 + bx[1,1] * 1.0      #Pr(O = 1 | G=2, C=0) = Pr(O=1 | A=1, C=1)
...
def test_post_gac():
    fwd_g = np.array([[1, .0, .0], [1.0, 0.0, 0], [0.25, .5, .25]])
    fwd_a = np.array([1.0, 1, 0.5])
    fwd_c = np.array([0.1, 0.4, 0.1, .6662])
...
def slug_bwd_p_o_given_g(bwd_gac, fwd_a, fwd_c, OBS2SNP, OBS2RG, n_obs):
    ...
    bwd_g[:, :, 0] = np.expand_dims(fwd_c[OBS2RG] * fwd_a[OBS2SNP] * bwd_gac[0, 2], 1) #alt cont
    bwd_g[:, :, 0] += np.expand_dims(fwd_c[OBS2RG] * (1-fwd_a[OBS2SNP]) * bwd_gac[0, 1], 1) #ref cont
    bwd_g[:, :, 1] = np.expand_dims(fwd_c[OBS2RG] * fwd_a[OBS2SNP] * bwd_gac[1, 2], 1) #alt cont
    bwd_g[:, :, 1] += np.expand_dims(fwd_c[OBS2RG] * (1-fwd_a[OBS2SNP]) * bwd_gac[1, 1], 1) #ref cont
...
def slug_post_g(bwd_g, fwd_g, N, O, 
                  OBS2SNP, n_snps):
...
@njit
def update_c(c, post_c, O, N, 
             OBS2SNP, OBS2RG, n_obs, n_rg):
    """update c
        parameters:
...
def test_update_c():
    pg = np.array([[1, .0, .0], [1.0, 0.0, 0], [0.25, .5, .25]])
    pa = np.array([1.0, 1, 0.5])
    O = np.array([1, 0, 3, 2, 3])
    N = np.array([1, 1, 5, 2, 6])
    e, b = 0.0, 0.00

    pc = np.array([0.1, 0.4, 0.1, .6662])
    ...
    c = update_c(pc, bwd_x, fwd_x_cont, fwd_x_nocont, O, N, 
                         IX.OBS2SNP, IX.OBS2RG, IX.n_obs, IX.n_rg)
...
class SlugPars(object):
    def __init__(self):
        self.cont = np.array((0, .1, 1, .4))
        self.e = 0
        self.b = .1
        self.tau = np.array((1, .1))
        self.F = np.array((0, 1))
...
def update_pars(pars, data):
    ...
    if do_update_cont:
        post_c = slug_post_c(bwd_x, fwd_x_nocont, fwd_x_cont, fwd_c,
                             IX.OBS2SNP, IX.OBS2RG, IX.n_obs)
        update_c(pars.cont, post_c, data.O, data.N, 
                 IX.OBS2SNP, IX.OBS2RG, IX.n_obs, IX.n_rg)


    if do_update_ftau:
        bwd_gac = slug_bwd_p_o_given_gac_simple(bwd_x) #size [3 x 2]
        bwd_g = slug_bwd_p_o_given_g(bwd_gac, fwd_a, fwd_c, IX.OBS2SNP, IX.OBS2RG,
                                     IX.n_obs)  #size [O x 3]
