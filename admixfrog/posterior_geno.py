...
def post_geno_py(P, cont, tau, IX, error):
    ...
    for snp, obs in zip(IX.OBS2SNP, ll_read):
        ll_snp[snp] *= obs
    ll_snp[IX.HAPSNP, 1] = 0.

    #P(G | Z, homo)
    for s in range(n_homo):
        prior_geno = _p_gt_homo_py(s, P, tau[s])
        prior_geno[IX.HAPSNP, 0] = P.beta[IX.HAPSNP,s] / (P.beta[IX.HAPSNP,s] + P.beta[IX.HAPSNP,s])
        prior_geno[IX.HAPSNP, 1] = 0
        prior_geno[IX.HAPSNP, 2] = P.alpha[IX.HAPSNP,s] / (P.beta[IX.HAPSNP,s] + P.beta[IX.HAPSNP,s])
        pg[:,s] =  ll_snp * prior_geno
    for s1 in range(n_homo):
        for s2 in range(s1+1, n_homo):
            s += 1
            #prior_geno = _p_gt_het_py(s1, s2, P)
            prior_geno = _p_gt_het_py2(P.alpha[:,s1], P.beta[:,s1],P.alpha[:,s2],P.beta[:,s2])
            pg[:,s] =  ll_snp * prior_geno
            pg[IX.HAPSNP, s] = 0

    pg = pg / np.sum(pg, 2)[:, :, np.newaxis]
    pg[np.isnan(pg)] = 1. / 3#n_states
...
def update_tau(tau, Z, pg, P, IX):
    ...
    for s in range(n_states):
        def f(t):
            x= np.log(_p_gt_homo_py(s, P, t[0])) * Z[IX.SNP2BIN,s][:, np.newaxis] * pg[:, s, :]
            x[IX.HAPSNP] = 0.
            return -np.sum(x)
        prev = f([tau[s]])
        OO =  minimize(f, [tau[s]], bounds=[(1e-4, 1e2)], method='L-BFGS-B', options=dict([("gtol",1e-2)]))
