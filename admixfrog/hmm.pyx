...
cdef void _get_emissions(
    ...
    for s in range(n_states):
        ...
        for i in range(n_snps):
            ...
            p = p * (1. - error) + (1.-p) * error
            row = bin_id[i]
            emissions[row, s] *= dbinom_single(O[i], N[i], p)
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
cdef void _get_emissions_beta_haploid(
    double[:, :] emissions,
    double [:] cont,
    long[:] O,
    long[:] N,
    ...
    for s in range(n_pops):
        for i in range(n_snps):
            ...
            for g in range(2):
                G = _dbetabinom_single(g, 1, a, b)
                p = cont[i] * p_cont[i] + (1.-cont[i]) / 2. * g
                p = p * (1. - error) + (1 - p) * error
                row = snp_id[i]
                emissions[row, s] += G  * dbinom_single(O[i], N[i], p)
    ...
    for s1 in range(n_pops):
        for s2 in range(s1+1, n_pops):
            ...
            for i in range(n_snps):
                ...
                D = G0  * dbinom_single(O[i], N[i], p0)
                D += G1  * dbinom_single(O[i], N[i], p1)
                D += G2  * dbinom_single(O[i], N[i], p2)
