...
class SlugParsLogit(object):
    """Parameters inferred by admixslug
    """
    def __init__(self, cont, tau, F, e = -7, b = -7.):
        n_rgs, n_sfs = len(cont), len(tau)
        assert len(tau) == len(F)
        ...
        self._pars[self.cont_slice] = logit(cont)
        self._pars[self.tau_slice] = logit(tau )
        self._pars[self.F_slice] = logit(F)
        self._pars[self.b_slice] = logit(b )
        self._pars[self.e_slice] = logit(e)
    ...
    @property
    def tau(self):
        return expit(self._pars[self.tau_slice])
    ...
    @property
    def F(self):
        return expit(self._pars[self.F_slice])
    ...
    @property
    def b(self):
        return expit(self._pars[self.b_slice])
    ...
    @property
    def e(self):
        return expit(self._pars[self.e_slice])
    ...
    @property
    def cont(self):
        return expit(self._pars[self.cont_slice])
    ...
    def logit(self):
        self._pars = logit(self._pars)

    def expit(self):
        self._pars = expit(self._pars)
