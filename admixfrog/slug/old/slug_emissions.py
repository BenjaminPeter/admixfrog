...
def reads_fwd_p_x(fwd_x_cont, fwd_x_nocont, fwd_c, data):
    ...
    res = (1 - fwd_c[data.READ2RG]) * fwd_x_nocont[data.READ2SNP] + fwd_c[data.READ2RG] * fwd_x_cont[data.READ2SNP]
...
def slug_bwd_p_o_given_gac(bx):
    ...
    """
    v = np.vstack(((1, .5, 0), (0, .5, 1))).T #Pr(X | G)
    return v @ bx

    res = np.empty((3, 2))
    res[0] = bx[0] * 1.0 + bx[1] * 0.0
    res[1] = bx[0] * 0.5 + bx[1] * 0.5
    res[2] = bx[0] * 0.0 + bx[1] * 1.0 #Pr(O | G= 2) = Pr(O | X = 0) Pr(X=0 | G=2) + Pr(O | X=1) Pr(X=1 | G=2)
...
def slug_bwd_p_one_o_given_g(bwd_x, fwd_a, fwd_c, OBS2SNP, OBS2RG, n_obs):
    ...
    bwd_g += np.expand_dims(bwd_g_nocont, 0) * np.expand_dims((1-fwd_c[OBS2RG]), (1,2))
...
def full_posterior_genotypes(data, pars):
    ...
    bwd_g1 = reads_bwd_p_one_o_given_g(bwd_x, data.psi, pars.cont, data.READ2SNP, data.READ2RG, data.n_reads)
...
def reads_full_posterior_genotypes(data, pars):
    ...
    bwd_g1 = slug_bwd_p_one_o_given_g(bwd_x, data.psi, pars.cont, data.OBS2SNP, data.OBS2RG, data.n_obs)
...
def full_posterior_cont(data, pars):
    ...
    return slug_post_c(bwd_x, fwd_x_nocont, fwd_x_cont, fwd_c, 
                       data.OBS2SNP, data.OBS2RG, data.n_obs)
...
def slug_post_c(bwd_x, fwd_x_nocont, fwd_x_cont, fwd_c, OBS2SNP, OBS2RG, n_obs):
    ...
    x_nocont_mat = np.vstack((1-fwd_x_nocont[OBS2SNP], fwd_x_nocont[OBS2SNP])). T @ bwd_x
