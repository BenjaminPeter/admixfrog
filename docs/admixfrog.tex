...
\begin{align}
P(G=2 | A,D, \tau, N) &= \int P(G|f) P(f | A, D, \tau, N) df\nonumber\\
&= \int \left[ F f + (1-F) f^2\right] P(f | A, D, \tau, N) df\nonumber\\
&= F E[f|A, D, N] + (1-F)  E[f^2|A, D, \tau, N]\nonumber\\
&= K(a\tau,d\tau,N) \left[F \frac{D }{A+D} + (1-F) \frac{(D) (D\tau+1)}{(A+D )(A\tau+D\tau+1)}\right]
\end{align}
Thus the probability will be decreased by a factor that depends on $a\tau, d\tau$ and $N$, but not $F$.
...
Thus
\begin{align*}
P(G=0) &= K_1' K_2'\frac{b_1}{a_1+b_1}\frac{b_2}{a_2 + b_2}\\
P(G=1) &=  \frac{K_1K_2'a_1b_2 + K_1'K_2a_2b_1}{(a_1+b_1)(a_2+b_2)}\\
P(G=2) &= K_1 K_2\frac{a_1}{a_1+b_1}\frac{a_2}{a_2 + b_2}
\end{align*}
...
As for $F$,
\begin{equation}
\hat{K} =\operatorname*{argmax}_K \left[ \sum_k\sum_{g=0}^2\sum_{l=1}^L \log P(G_t | Z_l=k, A_{lk}, B_{lk}, F)  P(G_l|Z_l, F', O) P(Z_l=k | O, \theta') 
\right]
\end{equation}
\begin{equation}
\hat{K} =\operatorname*{argmax}_K \left[ \sum_k\sum_{g=0}^2\sum_{l=1}^L \log K(A_{lk}, B_{lk}, n)  P(G_l|Z_l, F', O) P(Z_l=k | O, \theta') 
\right]
\end{equation}

but we need to optimize over all homo- and heterozygous states containing a particular $K$.
...
\end{align}
The term in the squared parentheses is also denoted as $V'$
