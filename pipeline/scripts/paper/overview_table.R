...
a_nea = read_csv("stats/frags/error2CAFR/state_NEA_0.2/NEA_DEN/5000/deni3_hcneaden.frags")
a_den = read_csv("stats/frags/error2CAFR/state_DEN_0.2/NEA_DEN/5000/deni3_hcneaden.frags")
h_nea = read_csv("stats/frags/error2CAFR/homo_NEA_0.2/NEA_DEN/5000/deni3_hcneaden.frags")
h_den = read_csv("stats/frags/error2CAFR/homo_DEN_0.2/NEA_DEN/5000/deni3_hcneaden.frags")

x_den = read_csv("stats/frags/error2CAFR/state_A_0.2/A=ALT+D12_D=DEN+D11/5000/deni3_hcneaden.frags")
y_den = read_csv("stats/frags/error2CAFR/state_D_0.2/A=ALT+D12_D=DEN+D11/5000/deni3_hcneaden.frags")
...
nice_names = rbind(
                   ...
                   c('mez1', 'Mezmayskaya 1'),
                   c('mez2', 'Mezmayskaya 2'),
                   c('scladina', 'Scladina'),
                   c('spy1', 'Spy 1'),
...
df0 = bind_rows(a_nea, a_den, h_nea, h_den, x_den, y_den) 
df = df0 %>%
    ...
    mutate(sex=chrom=='X') %>%
    select(sample, target, type, chrom, pos, pos_end, map, map_end, map_len, pos_len, sex)
...
frags = df %>% rowwise %>% 
    ...
    select(sample, target, type, chrom, pos, pos_end, map, map_end, pos_len, map_len, genes) 


frags %>% write_csv("tables/paper/all_frags.csv.gz")
...
tbl3 = frags %>% group_by(sample, target, type) %>% 
    rle_fit_grp(trunc=MIN_LENGTH) %>% mutate(gen=emean * 100, years=gen * 25) %>% 
    arrange(years) %>% 
    select(sample, years, gen, type, target) %>% left_join(tbl2, .) %>%
    left_join(read_tsv('config/age_ranges.tsv')) %>%
    mutate(se_years = years / sqrt(n_auto + n_sex), se_gen = gen / sqrt(n_auto + n_sex)) %>%
    right_join(nice_names, .) %>% select(-sample)  %>%
    mutate(Years = sprintf("%s±%s", round(years), round(2*se_years))) %>% 
...
tbl3[tbl3_order,] %>%  select(1, 3, 12, 22, 20, 21, 18, 19, 17, 16, 13) %>% 
    write_csv("tables/paper/raw.csv")
 
tbl4 = frags %>% group_by(sample, target, type) %>% 
    rle_fit_grp(trunc=MIN_LENGTH) %>% mutate(gen=emean * 100, years=gen * 25) %>% 
    arrange(years) %>% 
    left_join(tbl2, .) %>%
    mutate(se_years = years / sqrt(n_auto + n_sex), se_gen = gen / sqrt(n_auto + n_sex)) %>%
    right_join(nice_names, .) %>% select(-sample)  %>%
    mutate(target=factor(target, levels=c('NEA', 'DEN', 'D', 'A'))) 


tbl_d11 = df %>% filter(sample=='denisova11', map_len > 0.5, type=='homo') %>%
    select(chrom, pos, pos_end, map, map_end, map_len, pos_len) %>%
    mutate(pos = comma(pos), pos_end = comma(pos_end), pos_len = comma(pos_len))

tbl_d8_sum = df %>% filter(sample=='denisova8', map_len > 0.02, type=='homo') %>%
    tally

tbl_d8 = df %>% filter(sample=='denisova8', map_len > 10.0, type=='state', target=='NEA') %>%
    select(chrom, pos, pos_end, map, map_end, map_len, pos_len) %>%
    mutate(pos = comma(pos), pos_end = comma(pos_end), pos_len = comma(pos_len))
tbl_d8_homo = df %>% filter(sample=='denisova8', map_len > 1.0, type=='homo', target=='NEA') %>%
    select(chrom, pos, pos_end, map, map_end, map_len, pos_len) %>%
    mutate(pos = comma(pos), pos_end = comma(pos_end), pos_len = comma(pos_len))

tbl_d2 = df %>% filter(sample=='denisova2', map_len > 1.0, type=='state', target=='NEA') %>%
    select(chrom, pos, pos_end, map, map_end, map_len, pos_len) %>%
    mutate(pos = sprintf("chr%s:%s-%s", chrom, comma(pos), comma(pos_end)), pos_len = comma(pos_len)) %>%
    select(-pos_end)
