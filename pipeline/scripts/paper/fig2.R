...
P_cont = D_cont %>%
    ggplot(aes(x=sample, y=cont, fill=deam)) +
    geom_col(position='dodge') + 
    STUFF + 
    scale_y_continuous(expand=NONE, name='contamination', breaks=c(0, .5)) +
    coord_flip() +
    theme(axis.text.y=BLANK, axis.title.y=BLANK)
...
D_region = data %>%
    ...
    gather(variable, value, -sample:-n_snps, -age) %>%
...
P_region2 = D_region %>% 
    ggplot(aes(x=pos, y=value, fill=variable)) +
    geom_col(width=5000) + 
    facet_wrap(~sample, ncol=1, strip='l') +
    STUFF + 
    col_scale() +
...
col_lomax="blue"
P_fit = P_runs + 
    geom_line(data=fit_runs %>% filter(exp>1e-3), aes(y=exp), color=col_exp, lty=2, lwd=.5) + 
...
P = plot_grid(P_prop, P_fit, P_region, P_cov,
          align = 'h', nrow=1,
          axis='lb',
          rel_widths = c(2, 4, 4, 1.5)
          )
