...
plot_snp_run <- function(snp, ref, run, ext=c(4e5, 5e4),
    ...
    run_snp = snp %>% 
            filter(chrom==run$chrom, pos>=run$pos - ext[1], 
                   pos < run$pos_end + ext[2]) %>%
            mutate(in_region = pos >= run$pos & pos < run$pos_end) %>%
    ...
    x= lsr0 %>% 
        ...
        select(snp_id, bin, fixed, depth, pos, p_est=p, one_of(pops), p_read) %>% 
    ...
    if(plot_coverage){
        P = P + geom_text(aes(x=pos, y=large_top, label=depth), 
                          vjust=1, size=2, color='black', data=x %>% filter(k=='p_read'))
    } else {
        P = P + geom_hline(aes(yintercept=large_top), alpha=0, data = x %>% filter(k=='p_read'))
    }

    if(plot_est){
        P = P + geom_area(aes(x=pos, y=p_est, alpha=target), lwd=.6, color='black', data=x %>% filter(k=='p_read'))
    }
    return(P)
}
...
test2 <- function(){
    ...
    base="admixfrog/error2CAFR/20000/NEA_DEN/denisova8_hcneaden"
...
test3 <- function(){
    ...
    base="admixfrog/error2CAFR/20000/NEA_DEN/altai_hcneaden"
...
test4 <- function(){
    ref = read_csv("ref/ref_hcneaden.csv.xz", col_types=cols(chrom=col_character()))
    base="admixfrog/error2CEUR/20000/A=ALT+D12_D=DEN+D11/denisova3_hcneaden"
...
paper_d11 <- function(){
    ref = read_csv("ref/ref_hcneaden.csv.xz", col_types=cols(chrom=col_character()))
    base="admixfrog/error2CAFR/20000/NEA_DEN/denisova11_hcneaden"
    ...
    ggsave("figures/paper/longest/d11_run.png", P, width=8, height=2.2)
}
...
paper_d3 <- function(){
    ...
    runs = read_csv(sprintf("%s.rle.xz", base))
    snp = read_csv(sprintf("%s.snp.xz", base), col_types=cols(chrom=col_character()))
    run = runs %>% filter(target==TARGET, type=='state', chrom == '20' ) %>% arrange(-len) %>% head(1) %>% tail(1)
    #run = runs %>% filter(target==TARGET, chrom == '6') %>% arrange(-len) %>% head(1) %>% tail(1)
    ...
    ggsave("figures/paper/longest/d3_run.png", P, width=8, height=2.2)
}

paper_d2 <- function(){
    ref = read_csv("ref/ref_hcneaden.csv.xz", col_types=cols(chrom=col_character()))
    base="admixfrog/error2CAFR/20000/NEA_DEN/denisova2_hcneaden"
    ...
    P = plot_snp_run(snp, ref, run, 
                     ...
                     min_freq = 0.1,
                     one_snp_per_bin=F,
                     ext=c(2e6, 2e6), filter_ambiguous=T, 
                     pops=c("NEA", "DEN", "AFR"), base_pop='AFR')
    ggsave("figures/paper/longest/d2_run.png", P, width=8, height=2.9)
}

paper_d8 <- function(){
    ref = read_csv("ref/ref_hcneaden.csv.xz", col_types=cols(chrom=col_character()))
    base="admixfrog/error2CAFR/20000/NEA_DEN/vindija3319_hcneaden"
    ...
    P = plot_snp_run(snp, ref, run, 
                     ...
                     one_snp_per_bin=F,
                     ext=c(2e4, 1e4), 
                     pops=c("ALT", "DEN"), base_pop='ALT')
    ggsave("figures/paper/longest/v3319_run.png", P, width=8, height=2.9)
}
...
paper_vindija <- function(){
    ...
    P = plot_snp_run(snp, ref, run, 
                     ...
                     ext=c(4e6, 3e6), 
                     pops=c("NEA", "DEN", "AFR"), base_pop='AFR')
    ggsave("figures/paper/longest/d8_run.png", P, width=8, height=2.9)
}
...
paper_altai <- function(){
    ...
    ggsave("figures/paper/longest/d5_run.png", P, width=8, height=2.2)
}

paper_spy <- function(){
    ref = read_csv("ref/ref_hcneaden.csv.xz", col_types=cols(chrom=col_character()))
    base="admixfrog/error2CAFR/20000/NEA_DEN/spy1_hcneaden"
    ...
    P = plot_snp_run(snp, ref, run, 
                     ...
                     ext=c(1e5, 1e5), 
                     pops=c("NEA", "DEN"), base_pop='NEA')
    ggsave("figures/paper/longest/spy_run.png", P, width=8, height=2.2)
}
...
paper_mez1 <- function(){
    ...
    ggsave("figures/paper/longest/mez_run.png", P, width=8, height=2.2)
}
