...
panel1 = snakemake@wildcards$panel1
panel2 = snakemake@wildcards$panel2
rlefiles = snakemake@input$rle
binfiles = snakemake@input$bins

names = c(snakemake@config$panels[[panel1]], snakemake@config$panels[[panel2]])
print(names)
...
bins = load_bin_data(tail(binfiles, 1), tail(names,1))
