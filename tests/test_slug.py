...
def test_slug_p_gt_haploid():
    ...
    p_gt_haploid(tau=tau0, SNP2SFS=np.arange(10, dtype=int), res=res, FLIPPED=FLIPPED)
...
def test_slug_fwd_p_x():
    ...
    bwd_g1 = bwd_p_one_o_given_g(bwd_x, pa, pc, IX.READ2SNP, IX.READ2RG, IX.n_reads)
    ...
    assert res[0, 1] == 0.3 * 0.3 + 0.75 * 0.7
    assert res[1, 1] == 0.3
    assert res[2, 1] == 1


def test_slug_fwd_p_x2():
    fwd_g = np.array([[0.5, 0.5, 0]])
    pa = np.array([0.3, 1])
    pc = np.array([0.0])
...
def test_update_large():
    ...
    reps = (
        np.random.multinomial(n_obs - n_snps, pvals=np.zeros(n_snps) + 1.0 / n_snps) + 1
    )
    OBS2SNP = np.repeat(np.arange(n_snps, dtype=np.int), reps)

    p = true_cont[OBS2RG] * psi[OBS2SNP] + (1 - true_cont[OBS2RG]) * G[OBS2SNP] / 2.0
    p = e * (1.0 - p) + p * (1.0 - b)
    ...
    data = SlugData(
        REF=N - O, ALT=O, psi=psi, OBS2RG=OBS2RG, OBS2SNP=OBS2SNP, SNP2SFS=SNP2SFS
    )
